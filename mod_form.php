...
class mod_newsletter_mod_form extends moodleform_mod {
    ...
    public function definition() {
        ...
        $mform->addElement(
            'text',
            'name',
            get_string('newslettername', 'mod_newsletter'),
            ['size' => '64']
        );
        ...
        $mform->addElement(
            'filemanager',
            'stylesheets',
            get_string('stylesheets', 'mod_newsletter'),
            ['subdirs' => 0, 'maxbytes' => 0, 'maxfiles' => 0, 'accepted_types' => 'css']
        );
        $mform->addHelpButton('stylesheets', 'stylesheets', 'mod_newsletter');

        $mform->addElement(
            'advcheckbox',
            'allowguestusersubscriptions',
        ...
        $mform->addElement(
            'textarea',
            'welcomemessage',
            get_string('welcomemessage', 'mod_newsletter'),
        ...
        $mform->addElement(
            'select',
            'aboprofilefield',
            get_string('aboprofilefield', 'mod_newsletter'),
            $this->cpf_list()
        );
    ...
    public function make_subscription_option_list() {
        $options = [];
        $options[NEWSLETTER_SUBSCRIPTION_MODE_OPT_IN] = get_string(
            'sub_mode_opt_in',
            'mod_newsletter'
        );
        $options[NEWSLETTER_SUBSCRIPTION_MODE_OPT_OUT] = get_string(
            'sub_mode_opt_out',
            'mod_newsletter'
        );
        $options[NEWSLETTER_SUBSCRIPTION_MODE_FORCED] = get_string(
            'sub_mode_forced',
            'mod_newsletter'
        );
        return $options;
